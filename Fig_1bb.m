...
%%for algo_no = [1,2,3,4]
    ...
    for i = 1:itr
        ...
        %%if algo_no == 1
        %selected_movies=load('selected_movie_user_21.mat');
        %%[regret,reward,A] =
        %%CascadeLinTS(variance,d,n,K,movie_features,W_test,A_star,theta_star,w_movie);
       %% end
        %5if algo_no == 2
        %selected_movies=load('selected_movie_user_22.mat');
        [regret,reward,A] = CascadeLinUCB(variance,d,n,K,movie_features,W_test,A_star,theta_star,w_movie);
        %end
        %if algo_no == 3
        %selected_movies=load('selected_movie_user_23.mat');
        %[regret,reward,A] = RankedLinTS(variance,d,n,K,movie_features,W_test,A_star,theta_star,w_movie);
        %end
        %if algo_no == 4
        %selected_movies=load('selected_movie_user_24.mat');
        ...
        %end
        %if algo_no==6
        %[regret,reward,A] = CascadeLinUCB_NN(variance,d,n,K, movie_features,W_test,A_star,theta_star,w_movie,num_movies);
        %end   
    ...
    %% [r,h(c)]= boundedline(x,cum_regret,20,['-',color(c)]);
...
%%end
%title(['L = 256, K = 4'],'FontWeight','bold','FontSize',15);
